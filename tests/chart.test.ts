...
import { calculate, calculateDateOnly, validateInput, type SajuInput } from "../lib/saju/chart";
...
test("출생시간 없이 년·월·일주만 제공하고 시주는 제외한다", () => {
  const chart = calculateDateOnly("2005-12-23");
  assert.deepEqual(chart.pillars.map((item) => item.label), ["년주", "월주", "일주"]);
  assert.deepEqual(chart.pillars.map((item) => item.text), ["乙酉", "戊子", "辛巳"]);
  assert.equal(chart.pillars.some((item) => item.label === "시주"), false);
  assert.match(chart.method, /출생시간/);
});
