...
export function validateInput(raw: SajuInput): SajuInput {
  ...
  if (year < 1900)
    throw new InputError("1900년 1월 1일 이후의 날짜를 지원합니다.", "date");
...
// 출생시간이 없는 경우 정오 기준으로 년·월·일주만 참고합니다. 절기 경계의 정확한 시각은 알 수 없습니다.
export function calculateDateOnly(date: string) {
  ...
  return {
    pillars: chart.pillars.slice(0, 3),
    dayMaster: chart.dayMaster,
    method: "양력 생년월일의 정오를 기준으로 계산한 참고용 3주입니다. 출생시간이 없어 시주와 절기 경계의 정확한 판정은 제공하지 않습니다.",
  };
}
